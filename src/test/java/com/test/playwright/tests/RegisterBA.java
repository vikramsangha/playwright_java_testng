...
public class RegisterBA {
	...
	@Test (enabled=true)
	public void RegisterBAUser() throws InterruptedException {
		rp.enterEmailID(prop.getProperty("usernameBA").trim());
		lp = rp.navigateToLoginPage();
		Assert.assertEquals(rp.getPageTitle(), "TIBCO Accounts");
		lp.enterEmailID(prop.getProperty("usernameBA").trim());
		lp.clickNextButton();
		lp.enterPassword(prop.getProperty("passwordBA").trim());
